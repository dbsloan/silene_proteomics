...
#df <- read.csv("../Quantification/silene_psms.csv")
df <- read.csv("../Quantification/arabidopsis_psms.csv")

if (dim(df)[2]==7){
  df <- df[,2:7]
}
df <- df %>% mutate(across(everything(), ~log10(.x + 1)))
...
ggpairs(
  ...
  xlab ("log10 PSM Count") +
  ylab ("log10 PSM Count")

ggsave("Arabidopsis.CorrMatrix.PSM.pdf", width=6.7, height=6.7)
